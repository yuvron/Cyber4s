		...
		if (!availableMoves.includes(e.currentTarget)) {
			...
			if (chosenCell === e.currentTarget) chosenCell = undefined;
				...
				if (piece && piece.color === game.turn) {
					...
					chosenCell.classList.toggle("selected");
					const moves = piece.getMoves(game.board);
					if (moves) {
			...
			const lastId = chosenCell.id.split("-");
			const lastX = +lastId[0];
			const lastY = +lastId[1];
			const tryMove = game.movePiece(lastX, lastY, x, y); //movePiece returns false, true or a captured piece (which is truthy)
			if (tryMove) {
...
function addNotification(text) {
	...
	setTimeout(() => (notif.style.opacity = "0"), 0);
	const beep = () =>
		new Audio(
